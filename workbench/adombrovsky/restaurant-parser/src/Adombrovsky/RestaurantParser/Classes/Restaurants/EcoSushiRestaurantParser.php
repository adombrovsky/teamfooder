...
/**
 * Class EcoSushiRestaurantParser
...
class EcoSushiRestaurantParser implements IRestaurantParser
{
    ...
    const MAIN_HOST = 'http://eco-sushi.com.ua';
    ...
    public function parse ()
    {
        ...
        $restaurant = Restaurant::where('alias','=', Restaurant::ECOSUSHI_RESTAURANT)->first();
        $productIds = array();
        $categoryIds = array();
        foreach ($categories as $categoryAttributes)
        {
            ...
            foreach ($products as $productAttributes)
            {
                ...
                $productAttributes['url'] = $productAttributes['url'] ? $this->getMainHost().$productAttributes['url']:'';
